...
SELECT 
    ...
    ROUND((COUNT(DISTINCT user_id) * 100.0) / (SELECT COUNT(*) FROM users), 2) AS percentage_of_all_users
FROM
    feature_usage
GROUP BY feature;
...
SELECT
	COUNT(DISTINCT u.user_id) AS earners_over_60k,
    f.feature    
FROM
	users u
		JOIN
...
SELECT
	issue_type, AVG(DATEDIFF(resolved_at,created_at)) AS average_amount_of_days_to_resolution
FROM
	support_tickets
...
JOIN (
    SELECT 
        user_id,
        MIN(used_at) AS first_feature_use
    FROM feature_usage
    GROUP BY user_id
...
JOIN (
    SELECT 
        t.user_id,
        SUM(t.amount) AS total_spent_first_month
    FROM transactions t
    JOIN users u2 ON u2.user_id = t.user_id
    WHERE DATEDIFF(t.transaction_date, u2.join_date) <= 30
    GROUP BY t.user_id
) t ON u.user_id = t.user_id
WHERE 
    DATEDIFF(f.first_feature_use, u.join_date) <= 7
    AND t.total_spent_first_month > (
        SELECT 
            AVG(user_total)
        FROM (
            SELECT 
            ...
            WHERE DATEDIFF(t.transaction_date, u3.join_date) <= 30
            GROUP BY t.user_id
        ) first_month_spend
    );
